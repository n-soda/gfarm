...
HPCI_SECRET=K0WSOlQTizF7hJ7xOvGRDsD57ME0Vdxz
KEYCLOAK_USER=user
...
create_mapper()
{
	...
	${KCADM} delete client-scopes/${SCOPE_ID}/protocol-mappers/models/${MAPPER_ID} -r ${REALM}
	MAPPER_ID=""
    fi
    if [ -z "${MAPPER_ID}" ]; then
...
# cat <<EOF > /dev/null
# {
#   "id" : "20add110-d819-4d67-b970-7947c9c06233",
#   "name" : "hpci",
#   "protocol" : "openid-connect",
#   "attributes" : { },
#   "protocolMappers" : [ {
#     "id" : "f9d8e0a4-ebd8-41cc-9ea1-9194a20e2f4f",
#     "name" : "hpci.id",
#     "protocol" : "openid-connect",
...
#     }
#   }, {
#     "id" : "91cbe091-5e1f-47e0-9aae-8e2d84be247e",
#     "name" : "hpci.ver",
#     "protocol" : "openid-connect",
...
#     }
#   }, {
#     "id" : "3f949dff-d60a-479a-8243-810ecfeb0190",
#     "name" : "hpci-global",
#     "protocol" : "openid-connect",
...
delete_unnecessary_scope() {
    ...
    local found
    local exist=""
    for scope1 in $(${KCADM} get clients/${CLIENT_ID_ID}/${TARGET}-client-scopes -r ${REALM} | jq -r '.[] | .name'); do
        found=0
        ...
        if [ ${found} -eq 0 ]; then
            ...
            ${KCADM} delete clients/${CLIENT_ID_ID}/${TARGET}-client-scopes/${client_scope_id} -r ${REALM} > /dev/null
        fi
    done
    echo "${exist}"
}
...
add_scope() {
    ...
    for scope2 in ${SCOPES}; do
        ...
        if [ ${found} -eq 0 ]; then
            ...
            ${KCADM} update clients/${CLIENT_ID_ID}/${TARGET}-client-scopes/${client_scope_id} -r ${REALM}
        fi
    done
}
...
create_client_id "$CLIENT_ID_CONFIDENTIAL" \
  ...
  -s 'redirectUris=["http://localhost:8080/", "https://localhost:9443/*", "https://httpd.test/*", "http://httpd.test/*"]' \
  -s 'attributes."client.offline.session.idle.timeout"="86400"'\
  -s 'attributes."client.offline.session.max.lifespan"="31536000"'
update_client_scopes "${CLIENT_ID_CONFIDENTIAL}" "${DEFAULT_SCOPES}" "${OPTIONAL_SCOPES}"
...
# cat <<EOF > /dev/null
# }, {
#   "id" : "ecabf41f-9642-4c31-9637-d2088e722207",
#   "clientId" : "hpci-pub",
...
#   "defaultClientScopes" : [ "scitokens", "openid", "offline_access", "hpci" ],
...
# cat <<EOF > /dev/null
# {
#   "id" : "9ff803f7-e6b6-4e0c-a7d2-4fd8a9dcfbf3",
...
for i in $(seq 1 "$GFDOCKER_NUM_USERS"); do
  create_user $KEYCLOAK_USER$i $KEYCLOAK_PASSWORD
done
