 ...
 *
 * NOTE: kernel version won't need self_ip_count and self_ip_list
 */
#ifdef __KERNEL__

/* kernel version won't need self_ip_count and self_ip_list */

#else /* !__KERNEL__ */
static int self_ip_count = 0;
static struct in_addr *self_ip_list = NULL;
...
gfs_client_sockaddr_is_local(struct sockaddr *peer_addr)
{
	...
	int i;
	static pthread_once_t self_ip_asked = PTHREAD_ONCE_INIT;
	...
	pthread_once(&self_ip_asked, self_ip_init);
	...
	int failover_count)
{
	...
	if (e == GFARM_ERR_NO_ERROR)
		e = gfarm_auth_request(gfs_server->conn, GFS_SERVICE_TAG,
		    gfs_server->hostname, peer_addr, gfarm_get_auth_id_role(),
	...
	for (;;) {
		for (;;) {
			e = gfs_client_connection_acquire_by_host(
			    *gfm_serverp, canonical_hostname, port,
			    &gfs_server, source_ip);
			if (e == GFARM_ERR_NO_ERROR)
				break;
	...
	struct gfs_client_connect_state **statepp)
{
	...
	if (connection_in_progress) {
		...
		e = gfarm_auth_request_multiplexed(q,
		    ...
		    gfarm_get_auth_id_role(), user, NULL, connect_timeout,
		    gfs_client_connect_end_auth, state,
		    &state->auth_state);
		if (e == GFARM_ERR_NO_ERROR) {
			*statepp = state;
