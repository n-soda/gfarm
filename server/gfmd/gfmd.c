	...
	gfarm_int32_t *requestp, gfarm_error_t *on_errorp, int *suspendedp)
{
	...
	*requestp = request;
	...
	case GFM_PROTO_USER_INFO_GET_BY_GSI_DN:
		...
		break;
	case GFM_PROTO_USER_INFO_GET_MY_OWN:
		e = gfm_server_user_info_get_my_own(peer, from_client, skip);
		break;
	case GFM_PROTO_USER_INFO_GET_BY_AUTH_ID:
		e = gfm_server_user_info_get_by_auth_id(peer,
				from_client, skip);
		break;
	...
	case GFM_PROTO_PROCESS_FD_INFO:
		...
		break;
	case GFM_PROTO_PROCESS_FD_REMOVE:
		e = gfm_server_process_fd_remove(peer, from_client, skip);
		break;
...
boot_apply_db_journal(void)
{
	...
	/*
	 * use the async_commit variable to explicitly show it's not changed.
	 * although the gfarm_metadb_backend_sync_commit_at_initialization
	 * setting shouldn't be changed during this function
	 */
	int async_commit = !gfarm_metadb_backend_sync_commit_at_initialization;
...
transform_to_master(void)
{
	...
	journal_log_verbose_set(1); /* begin debugging slow transformation */
