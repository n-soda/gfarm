...
#	define BUFSIZE_MAX 2048
	...
	if ((op & GFS_W_OK) != 0 && read_only) {
		gflog_debug(GFARM_MSG_1005181, "%s (%s@%s) parent %llu:%llu"
	...
	if ((created || (op & GFS_W_OK) != 0 ||
		...
		if (inode_get_replica_spec(inode, &repattr, &desired_number) ||
			...
			(void)process_record_replica_spec(
			    process, peer, fd, desired_number, repattr, diag);
	}
...
gfm_server_futimes(struct peer *peer, int from_client, int skip)
{
	...
	if (!from_client && (spool_host = peer_get_host(peer)) == NULL) {
		...
		gflog_debug(GFARM_MSG_1005182, "%s (%s@%s) for inode %llu:%llu"
		    " during read_only",
		    diag, peer_get_username(peer), peer_get_hostname(peer),
...
gfm_server_fchmod(struct peer *peer, int from_client, int skip)
{
	...
	if (!from_client && (spool_host = peer_get_host(peer)) == NULL) {
		...
		gflog_debug(GFARM_MSG_1005183, "%s (%s@%s) for inode %llu:%llu"
		    " during read_only",
		    diag, peer_get_username(peer), peer_get_hostname(peer),
		    (long long)inode_get_number(inode),
		    (long long)inode_get_gen(inode));
...
gfm_server_fchown(struct peer *peer, int from_client, int skip)
{
	...
	if (!from_client && (spool_host = peer_get_host(peer)) == NULL) {
		...
		gflog_debug(GFARM_MSG_1005184, "%s (%s@%s) for inode %llu:%llu"
		    " during read_only",
		    diag, peer_get_username(peer), peer_get_hostname(peer),
		    (long long)inode_get_number(inode),
		    (long long)inode_get_gen(inode));
...
gfm_server_cksum_set(struct peer *peer, int from_client, int skip)
{
	...
	if ((process = peer_get_process(peer)) == NULL) {
		...
		gflog_debug(GFARM_MSG_1005185, "%s (%s@%s) for inode %llu:%llu"
		    " during read_only",
		    diag, peer_get_username(peer), peer_get_hostname(peer),
		    (long long)inode_get_number(inode),
		    (long long)inode_get_gen(inode));
...
gfm_server_remove(struct peer *peer, int from_client, int skip)
{
	...
	if ((process = peer_get_process(peer)) == NULL) {
		...
		gflog_debug(GFARM_MSG_1005186, "%s (%s@%s) for "
...
gfm_server_rename(struct peer *peer, int from_client, int skip)
{
	...
	if ((process = peer_get_process(peer)) == NULL) {
		...
		gflog_debug(GFARM_MSG_1005187, "%s (%s@%s) from "
...
gfm_server_flink(struct peer *peer, int from_client, int skip)
{
	...
	if (!from_client && (spool_host = peer_get_host(peer)) == NULL) {
		...
		gflog_debug(GFARM_MSG_1005188, "%s (%s@%s) for inode %llu:%llu"
		    " parent %llu:%llu name %s during read_only",
		    diag, peer_get_username(peer), peer_get_hostname(peer),
...
gfm_server_mkdir(struct peer *peer, int from_client, int skip)
{
	...
	if ((process = peer_get_process(peer)) == NULL) {
		...
		gflog_debug(GFARM_MSG_1005189, "%s (%s@%s) for "
		    "parent %llu:%llu name %s during read_only",
		    diag, peer_get_username(peer), peer_get_hostname(peer),
		    (long long)inode_get_number(base),
		    (long long)inode_get_gen(base), name);
...
gfm_server_symlink(struct peer *peer, int from_client, int skip)
{
	...
	if (!from_client && (spool_host = peer_get_host(peer)) == NULL) {
		...
		gflog_debug(GFARM_MSG_1005190, "%s (%s@%s) for %s to "
		    "parent %llu:%llu name %s during read_only",
		    diag, peer_get_username(peer), peer_get_hostname(peer),
		    source_path,
		    (long long)inode_get_number(base),
...
reopen_resume(struct peer *peer, void *closure, int *suspendedp)
{
	...
	if ((process = peer_get_process(peer)) == NULL) {
		...
		gflog_debug(GFARM_MSG_1005191, "%s (%s@%s) for inode %llu:%llu"
	...
	int *suspendedp)
{
	...
	if (!from_client && (spool_host = peer_get_host(peer)) == NULL) {
		...
		gflog_debug(GFARM_MSG_1005192, "%s (%s@%s) for inode %llu:%llu"
		    "flags 0x%x during read_only",
		    diag, peer_get_username(peer), peer_get_hostname(peer),
		    (long long)inum, (long long)gen, flags);
		e = GFARM_ERR_READ_ONLY_FILE_SYSTEM;
	...
	char **trace_logp)
{
	...
	if (from_client) { /* from gfsd only */
		...
		e = process_get_file_opening(process, peer, fd, &fo, diag);
		if (e != GFARM_ERR_NO_ERROR) {
			gflog_info(GFARM_MSG_1005195,
			    "%s (%s@%s) bad descriptor %d: %s", diag,
			    peer_get_username(peer), peer_get_hostname(peer),
			    fd, gfarm_error_string(e));
		} else {
			gflog_notice(GFARM_MSG_1005196,
			    "%s (%s@%s) for inode %llu:%llu flag 0x%x "
			    "during read_only",
			    diag,
			    peer_get_username(peer), peer_get_hostname(peer),
...
fhclose_write_resume(struct peer *peer, void *closure, int *suspendedp)
{
	...
	if ((spool_host = peer_get_host(peer)) == NULL) {
		...
		gflog_warning(GFARM_MSG_1004028,
	...
	if (e == GFARM_ERR_NO_ERROR && gfarm_ctxp->file_trace &&
	    (flags & GFM_PROTO_CLOSE_WRITE_GENERATION_UPDATE_NEEDED) != 0 &&
	    trace_log != NULL) {
		gflog_trace(GFARM_MSG_1004029, "%s", trace_log);
		free(trace_log);
	}
	...
	gfarm_uint64_t *cookiep, int *suspendedp, const char *diag)
{
	...
	if (e == GFARM_ERR_NO_ERROR && gfarm_ctxp->file_trace &&
	    ...
	    trace_log != NULL) {
		gflog_trace(GFARM_MSG_1004031, "%s", trace_log);
		free(trace_log);
	}
	return (e);
}
	...
	const char *diag)
{
	...
	if (from_client) { /* from gfsd only */
		...
		if (inode == NULL) {
			gflog_info(GFARM_MSG_1005198, "%s (%s@%s) "
			...
			gflog_notice(GFARM_MSG_1005199, "%s (%s@%s) "
			    "cookie %llu for inode %llu:%llu during read_only",
			    diag,
			    peer_get_username(peer), peer_get_hostname(peer),
			    (long long)cookie,
		...
		if (e != GFARM_ERR_NO_ERROR) {
			gflog_warning(GFARM_MSG_1004033,
			    "%s: inode %lld:%lld on host %s, cookie %lld: "
			    "new generation for size %lld -> %lld - "
			    "wakeup(%s): %s\n",
			    diag,
			    (long long)inode_get_number(inode),
			    (long long)inode_get_gen(inode),
			...
			if (result == GFARM_ERR_CONFLICT_DETECTED)
				gflog_warning(GFARM_MSG_1004034,
				    "%s: inode %lld:%lld on host %s, "
				    "cookie %lld: new generation rename: "
...
gfarm_error_t
gfm_server_config_get(struct peer *peer, int from_client, int skip)
{
	gfarm_error_t e;
	...
	static const char diag[] = "GFM_PROTO_CONFIG_GET";
	...
	if (e != GFARM_ERR_NO_ERROR) {
		...
		e = gfm_server_put_reply(peer, diag, e, "ci", fmt, storage.i);
	} else if (fmt == 's') {
		e = gfm_server_put_reply(peer, diag, e, "cs", fmt,
		    storage.s != NULL ? storage.s : "");
		free(storage.s);
	} else {
		assert(0);
	}
...
gfm_server_config_set(struct peer *peer, int from_client, int skip)
{
	...
	if (e == GFARM_ERR_NO_ERROR)
		...
		gflog_notice(GFARM_MSG_1005200,
...
gfm_server_replica_remove_by_file(struct peer *peer, int from_client, int skip)
{
	...
	if ((process = peer_get_process(peer)) == NULL) {
		...
		gflog_debug(GFARM_MSG_1005201, "%s (%s@%s) for "
...
gfm_server_replicate_file_from_to(struct peer *peer, int from_client, int skip)
{
	...
	if ((process = peer_get_process(peer)) == NULL)
		...
		if (e != GFARM_ERR_NO_ERROR) {
			gflog_info(GFARM_MSG_1005202,
			    "%s (%s@%s) bad descriptor %d: %s", diag,
			    peer_get_username(peer), peer_get_hostname(peer),
			    cfd, gfarm_error_string(e));
		} else {
			gflog_debug(GFARM_MSG_1005203,
...
replica_adding_resume(struct peer *peer, void *closure, int *suspendedp)
{
	...
	if ((spool_host = peer_get_host(peer)) == NULL) {
		...
		e = process_get_file_opening(process, peer, arg->fd,
		    &fo, diag);
		if (e != GFARM_ERR_NO_ERROR) {
			gflog_info(GFARM_MSG_1005204,
			    "%s (%s@%s) bad descriptor %d: %s", diag,
			    peer_get_username(peer), peer_get_hostname(peer),
			    arg->fd, gfarm_error_string(e));
		} else {
			gflog_debug(GFARM_MSG_1005205,
	...
	int cksum_protocol, const char *diag, int *suspendedp)
{
	...
	if (from_client) { /* from gfsd only */
		...
		e = process_get_file_opening(process, peer, fd, &fo, diag);
		if (e != GFARM_ERR_NO_ERROR) {
			gflog_info(GFARM_MSG_1005206,
			    "%s (%s@%s) bad descriptor %d: %s", diag,
			    peer_get_username(peer), peer_get_hostname(peer),
			    fd, gfarm_error_string(e));
		} else {
			gflog_debug(GFARM_MSG_1005207,
			    "%s (%s@%s) for inode %llu:%llu to %s "
			    "during read_only",
			    diag,
			    peer_get_username(peer), peer_get_hostname(peer),
	...
	gfarm_uint32_t cksum_result_flags)
{
	...
	if (from_client) { /* from gfsd only */
		...
		e = process_get_file_opening(process, peer, fd, &fo, diag);
		if (e != GFARM_ERR_NO_ERROR) {
			gflog_info(GFARM_MSG_1005208,
			    "%s (%s@%s) bad descriptor %d: %s", diag,
			    peer_get_username(peer), peer_get_hostname(peer),
			    fd, gfarm_error_string(e));
		} else {
			gflog_warning(GFARM_MSG_1005209,
...
gfm_server_replica_added_cksum(struct peer *peer, int from_client, int skip)
{
	...
	e = gfm_server_get_request(peer, diag, "iiilsbi",
	    ...
	    &cksum_result_flags);
	if (e != GFARM_ERR_NO_ERROR) {
		gflog_debug(GFARM_MSG_1004035, "%s request failed: %s",
			diag, gfarm_error_string(e));
		return (e);
	}
	if (skip) {
...
gfm_server_replica_lost(struct peer *peer, int from_client, int skip)
{
	...
	if (from_client) { /* from gfsd only */
		...
		gflog_debug(GFARM_MSG_1005210, "%s (%s@%s) for "
		    "inode %llu:%llu during read_only",
		    diag, peer_get_username(peer), peer_get_hostname(peer),
...
gfm_server_replica_add(struct peer *peer, int from_client, int skip)
{
	...
	if (from_client) { /* from gfsd only */
		...
		gflog_debug(GFARM_MSG_1005211, "%s (%s@%s) for "
	...
	int from_client, int skip)
{
	...
	if (from_client) { /* from gfsd only */
		...
		gflog_debug(GFARM_MSG_1005212, "%s (%s@%s) for "
...
gfarm_error_t
gfm_server_replica_open_status(struct peer *peer, int from_client, int skip)
{
	gfarm_error_t e;
	struct host *spool_host;
	gfarm_ino_t inum;
	...
	if (from_client) { /* from gfsd only */
		...
		open_status = inode_get_open_status_by_host(inode, spool_host);
...
gfm_server_fhset_cksum(struct peer *peer, int from_client, int skip)
{
	...
	if (from_client) { /* from gfsd only */
		...
		gflog_debug(GFARM_MSG_1005213, "%s (%s@%s) for "
